...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$password || !$nim || !$nama || !$angkatan || !$jurusan || !$email || !$no_hp) {
        ...
        if ($stmt->fetch()) {
            ...
            $stmt = $pdo->prepare("SELECT id_alumni FROM alumni WHERE email = ?");
            $stmt->execute([$email]);
            if ($stmt->fetch()) {
                ...
                try {
                    ...
                    // Insert alumni
                    $stmt = $pdo->prepare("INSERT INTO alumni (nim, nama, angkatan, jurusan, email, no_hp, pekerjaan, perusahaan, alamat) VALUES (?,?,?,?,?,?,?,?,?)");
                    $stmt->execute([$nim, $nama, $angkatan, $jurusan, $email, $no_hp, $pekerjaan, $perusahaan, $alamat]);
                    $id_alumni = $pdo->lastInsertId();

                    // Insert user dengan status pending
                    $hashed = password_hash($password, PASSWORD_DEFAULT);
                    $stmt = $pdo->prepare("INSERT INTO users (username, password, role, id_alumni, status) VALUES (?,?,'user',?,'pending')");
                    $stmt->execute([$username, $hashed, $id_alumni]);
  ...
  <div class="auth-left">
    ...
    <div class="reg-steps">
      ...
      <div class="step active">
        ...
        <div class="step-text">Data Alumni</div>
      </div>
      <div class="step-line"></div>
      <div class="step">
    ...
    <div class="auth-quote">
      <p>"Daftar sekarang dan terhubung dengan sesama alumni."</p>
    </div>
  </div>
  ...
  <div class="auth-right">
    <div class="auth-card auth-card-wide">
      ...
      <form method="POST" class="auth-form">
        ...
        <div class="form-row">
          <div class="form-group">
            <label>NIM / No. Induk <span class="req">*</span></label>
            <div class="input-wrapper">
              <svg class="input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/>
              </svg>
              <input type="text" name="nim" placeholder="Nomor Induk Siswa" required value="<?= htmlspecialchars($_POST['nim'] ?? '') ?>">
            </div>
          </div>
          <div class="form-group">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <svg class="input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.18 2 2 0 0 1 3.59 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.73a16 16 0 0 0 6 6l.91-.91a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
              </svg>
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            <svg class="input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="top:14px">
